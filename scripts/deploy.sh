...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "       If loader reports ERROR (CMV already enabled, LoadProcessStatus=None):"
    echo "         Redeploy DMs as Draft to unlock CMV, then re-run loader:"
    echo "         sf project deploy start --metadata DecisionMatrixDefinition --target-org $TARGET_ORG --test-level NoTestRun --wait 30"
    echo "         python3 scripts/load_decision_matrix_rows.py --org $TARGET_ORG --activate-es"
    echo ""
