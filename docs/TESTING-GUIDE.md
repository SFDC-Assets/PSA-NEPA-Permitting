...
**Test suite size:** 67 test classes, 700+ test methods across all feature areas. Run `sf apex run test --test-level RunLocalTests` to execute the full automated suite (see [Section 20](#20-apex-test-suite)).
...
19. [PIC Enum Conformance and Submission Narrative Claims](#19-pic-enum-conformance-and-submission-narrative-claims)
...
2. Ensure the comment period is open: set `nepa_public_comment_period_start__c` to yesterday and `nepa_public_comment_period_end_date__c` to 30 days from now on the IA record.
3. Navigate to the related **Public Complaints** (Comments) related list and click **New**.
4. Set:
   - **Commenter Organization** (`ReporterOrganization`): `Navajo Nation`
...
**Expected:** Save is blocked with a message referencing tribal consultation certification (EO 13175 / NHPA Section 106).
...
**Expected:** Stage gate passes the tribal consultation check (other gates may still block — this confirms VR-005 specifically is cleared).
...
## 18. CEQ v1.2 Compliance Test Suite

`NepaCeqComplianceTest` is a comprehensive compliance harness (38 test methods) that validates both REST export services end-to-end against the CEQ NEPA and Permitting Data and Technology Standard v1.2. It is the definitive test for confirming the data model and export services meet MFR submission requirements.

### 18a. Run the Compliance Test Suite

```bash
sf apex run test \
  --class-names NepaCeqComplianceTest \
  --target-org $ALIAS \
  --result-format human \
  --code-coverage \
  --wait 15
...
**Expected:** All 38 tests pass. The suite covers:

| Category | Tests | What is verified |
|---|---|---|
| Envelope structure | 2 | `schema_version = "1.2"`, full-graph response shape |
| CEQ property names | 3 | `project` uses `project_id`/`project_title`/`lead_agency`; process uses `federal_unique_id`/`nepa_review_type`; no camelCase leak |
| Picklist conformance | 2 | `nepa_review_type` ∈ `{EIS, EA, CE, Other}` (PIC wire values); `status` ∈ `{planned, underway, paused, completed}` (PIC wire values) |
| EIS lifecycle completeness | 3 | Start date, completion date, comment period dates all present on completed EIS record |
| Document DTO | 1 | Document DTO contains `id`, `document_title`, `document_type`, `status`, `publication_date` |
| Comment DTO keys | 1 | Comment DTO contains `id`, `comment_body`, `submission_method`, `is_substantive`, `organization`, `sentiment`, `response_status`, `agency_response` |
| Submission method conformance | 2 | Valid per PIC v1.2: `{Federal Register, Regulations.gov, Email, Mail, In Person, Web Portal, Other}`; all 3 test comments use distinct valid methods |
| Full-graph nesting | 4 | `documents`, `public_engagement_events`, `case_events`, `permits` arrays present; `comments` nested under `documents` not at process level |
| Engagement DTO | 2 | Engagement contains `id`, `event_type`, `start_datetime`, `end_datetime`, `location`, `attendance_count`, `public_access`, `notes`; `event_type` = `"Public Hearing"` round-trips |
| Case event DTO | 2 | Case event contains `id`, `event_type`, `event_date`, `description`; count matches inserted records |
| Provenance — all 6 entity types | 6 | `data_record_version`, `data_source_agency`, `data_source_system`, `record_owner_agency`, `retrieved_timestamp`, `last_updated` present and non-null on process, document, comment, engagement, case_event, and project entities |
| Null provenance fallback | 1 | All 6 provenance keys present even when source fields are null (keys present with null/empty values is valid MFR behavior) |
| GIS data | 1 | `gis_data` at project level contains `id`, `layer_type`, `geometry_type`, `coordinates` |
| Permit snake_case and sort | 2 | Permit DTO contains `permit_type`, `permit_status`, `is_critical_path`, `regulatory_citation`; critical-path permits sort before non-critical |
| All-entities completeness | 2 | Process node has all 7 child arrays; total entity counts match inserted records |
| Per-process export envelope | 2 | `NepaCeqExportService` `success` = true, `data` is array, process has `data_record_version` and `record_owner_agency` |

### 18b. Key test setup

`@TestSetup` builds a complete FAA EIS scenario:

- Program (`SAMPLE-FAA-EIS-001`) — Transportation-Aviation, all 6 v1.2 provenance fields
- IndividualApplication — `nepa_review_type__c = EIS`, `nepa_process_status__c = completed`, 3 cooperating agencies
- 4 ContentVersion documents (NOI, Draft EIS, Final EIS, ROD)
- 3 PublicComplaints — one per valid submission method (Regulations.gov, Federal Register, Email)
- 4 nepa_engagement__c records (2 scoping, 1 public hearing, 1 coordination)
- 9 ApplicationTimeline case events (full EIS lifecycle)
- nepa_gis_data__c record and nepa_process_team_member__c record
- 2 nepa_required_permit__c records (one critical-path, one non-critical)
- Separate `NULL-PROV-001` program/process with all provenance fields null (for null-provenance fallback tests)

### 18c. If a compliance test fails

| Failure | Likely cause |
|---|---|
| `nepa_review_type` not in valid set | `nepa_review_type__c` picklist value missing or mapped incorrectly in `toDto()` |
| `status` not in valid set | `nepa_process_status__c` picklist value doesn't match CEQ v1.2 lowercase values |
| `submission_method` not in valid set | `Method_of_Submission__c` picklist or the export service mapping doesn't cover the new value |
| Provenance key missing on an entity | A new entity type was added to the export without the 6-field provenance treatment |
| Comments at process level instead of under documents | `NepaCeqFullExportService` comment grouping logic; comments should be nested under their parent document via `nepa_parent_document__c` |
| `schema_version` not `"1.2"` | Hardcoded value in `NepaCeqFullExportService` or `NepaCeqExportService` changed |

---

## 19. PIC Enum Conformance and Submission Narrative Claims

Two dedicated test classes verify that (a) both export services emit PIC v1.2-conformant enum values on the wire and (b) every auditable claim in `docs/SUBMISSION-NARRATIVE.md` has automated coverage.

### 19a. Run PIC Conformance + Narrative Claims Tests

```bash
sf apex run test \
  --class-names NepaCeqStandardsConformanceTest \
  --class-names NepaSubmissionNarrativeClaimsTest \
  --target-org $ALIAS \
  --result-format human \
  --wait 15
```

**Expected:** All 33 tests pass (16 conformance + 17 narrative claims).

### 19b. NepaCeqStandardsConformanceTest (16 tests)

Verifies PIC v1.2 JSON schema enum constraints at the export wire layer.

| Category | Tests | What is verified |
|---|---|---|
| Process status mapping | 6 | `"in progress"` → `"underway"`, `"pre-application"` → `"planned"`, `"cancelled"` → `"completed"`, `"planned"/"paused"/"completed"` pass through unchanged |
| All statuses in PIC enum | 1 | Every exported process status is in `{planned, underway, paused, completed}` |
| Review type mapping | 2 | `"Other Authorization"` → `"Other"`, `"EA"` unchanged |
| All review types in PIC enum | 1 | Every exported type is in `{EIS, EA, CE, Other}` |
| Raw values preserved | 2 | `other.raw_status` = `"in progress"`, `other.raw_review_type` = `"Other Authorization"` |
| Project status (full export) | 2 | `"in progress"` → `"underway"`, `"pre-application"` → `"pre-application"` via full graph export |
| All project statuses in PIC enum | 1 | Every exported project status is in `{pre-application, underway, paused, completed}` |
| Case event datetime | 1 | `case_event.datetime` is present and matches `yyyy-MM-dd'T'HH:mm:ss'Z'` ISO 8601 format |

**Architecture note:** PIC mapping is applied at the export layer — raw Salesforce picklist values remain unchanged in the operational database and are preserved in the `other{}` block so dashboards and flows continue to function. This is intentional: the Salesforce model retains its richer vocabulary while the API wire conforms to the PIC standard.

### 19c. NepaSubmissionNarrativeClaimsTest (17 tests)

Verifies auditable claims in `docs/SUBMISSION-NARRATIVE.md` by section.

| MFR/Claim | Tests | What is verified |
|---|---|---|
| MFR #1 — OFD milestones | 2 | `NEPA_OFD_Milestone__mdt` contains exactly 8 records; all 8 E.O. 13807 DeveloperNames present |
| MFR #5 — permit gap penalty | 6 | `nepa_blocked_permit_count__c` rollup: 1 blocked = 1, 3 blocked = 3, Issued + non-critical = 0; scorer fires without exception at +8 and +15 thresholds; decrement on Issued transition |
| MFR #6 — CUI tagging fields | 5 | `nepa_sensitivity_classification__c` persists `Unclassified`, `CUI`, `CUI//SP-TRIBAL`; `nepa_data_access_restriction__c` persists FOIA text; both fields populate together |
| Tech — picklist metadata | 2 | `nepa_process_status__c` and `nepa_current_status__c` picklist describes contain `"underway"` (PIC v1.2 native value) |
| Tech — GIS purpose/format | 2 | `"Base Map"` purpose inserts successfully; all 7 PIC format values (`GeoJSON`, `KML`, `Shapefile`, `GML`, `GeoTIFF`, `WKT`, `Other`) are valid picklist values |

### 19d. If a conformance test fails

| Failure | Likely cause |
|---|---|
| Exported status not in PIC enum | Mapping table in `NepaCeqExportService` or `NepaCeqFullExportService` missing a value |
| `other.raw_status` missing | `toDto()` / `processDto()` `other{}` block omits `raw_status` key |
| `case_event.datetime` null or wrong format | `datetimeStr` in `caseEventDtos()` — check `nepa_retrieved_timestamp__c` is set in test setup |
| `nepa_blocked_permit_count__c` wrong | Roll-up summary field not updating; check field criteria in object definition |
| `"underway"` not in picklist describe | Re-deploy `force-app/main/default/objects/IndividualApplication` and `objects/Program` to the target org |
| OFD milestone count ≠ 8 | CMT records not deployed; deploy `force-app/main/default/customMetadata/` |
...
**Expected:**
- **All tests pass**, 0 failures. The current baseline is 887+ test methods across 67 test classes; the exact count increases as tests are added. When using `--result-format json`, a passing run shows `"summary": { "outcome": "Passed", "failing": 0 }` and the `summary.passing` field shows the current count.
- Overall Apex coverage ≥ 75%
- Key test classes that must pass:
  - `NepaApiComplianceTest` (55 tests)
  - `NepaBREConfigTest` (46 tests)
  - `NepaCeqComplianceTest` (38 tests — end-to-end CEQ v1.2 compliance: picklists, provenance on all 6 entities, DTO shapes, nesting, sort order)
  ...
  - `NepaCeqStandardsConformanceTest` (16 tests — PIC v1.2 enum conformance at export wire; status/type mapping; raw value preservation; ISO 8601 datetime)
  - `NepaSubmissionNarrativeClaimsTest` (17 tests — auditable claims: OFD milestones, permit gap penalty, CUI fields, picklist metadata, GIS formats)
  - `NepaEntity789Test` (25 tests)
  - `NepaStageGateTest` (19 tests)
...
| `NepaCeqComplianceTest` | 38 | End-to-end CEQ v1.2 compliance: schema version, CEQ property names, picklist conformance (review type, status, submission method), EIS lifecycle dates, all entity DTOs (document, comment, engagement, case_event, permit), provenance on all 6 entity types, null provenance fallback, GIS data, nesting structure, per-process envelope |
...
| `NepaCeqStandardsConformanceTest` | 16 | PIC v1.2 enum conformance: status/type mapping at export wire, raw values in `other{}`, project status full-export mapping, ISO 8601 case event datetime |
| `NepaSubmissionNarrativeClaimsTest` | 17 | Submission Narrative MFR claims: OFD 8-milestone count+names, permit gap rollup thresholds, CUI tagging fields, `"underway"` picklist describe, GIS format values |
...
| `NepaStageGateTest` | 19 | Before-save stage gate validation rules (VR-001 tiering, VR-004 ESA, VR-005 tribal) |
...
**Expected:** All 19 tests pass, including VR-001 (supplementation gate), VR-004 (ESA consultation gate), VR-005 (tribal consultation gate), error message content assertions (`1502.9`, `ESA consultation`, and `tribal`/`Consultation`), stage transition timestamp stamping, and target completion date auto-population.
...
| 20a | Test Suite | All tests pass (887+ methods, 0 failures), ≥ 75% coverage | | |
...
| 18a | CEQ Compliance | NepaCeqComplianceTest (38 tests) passes — picklists (PIC wire values), provenance, DTO shapes, nesting | | |
| 19a | PIC Conformance | NepaCeqStandardsConformanceTest (16 tests) passes — status/type enum mapping, raw value preservation | | |
| 19a | Narrative Claims | NepaSubmissionNarrativeClaimsTest (17 tests) passes — OFD milestones, permit gap, CUI fields | | |
| 20e | Test Suite | NepaCeqExportServiceTest (51 tests) passes | | |
...
| 20j | Test Suite | NepaStageGateTest (19 tests) passes | | |
...
| Tribal flag not set | `NEPA_Plaintiff_Profile__mdt` has no entry for the commenter org | Add org to CMT or check spelling exactly matches `ReporterOrganization` |
