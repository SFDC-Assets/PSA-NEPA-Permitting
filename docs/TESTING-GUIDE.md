...
**Test suite size:** 36 test classes, 473+ test methods across all feature areas. Run `sf apex run test --test-level RunLocalTests` to execute the full automated suite (see [Section 20](#20-apex-test-suite)).
...
| `NepaLitigationRiskScorerTest` | 17 | BRE risk scoring, tier thresholds |
...
| `NepaPlaintiffIntelligenceTest` | 23 | Plaintiff risk flag, tribal dual-flag, comment-level flags, Idaho Conservation League / Shoshone-Paiute CMT |
...
| `NepaChallengePredictorTest` | 9 | Challenge prediction rules, nepa_challenge_prediction_basis__c |
...
| 20a | Test Suite | All 473+ tests pass, ≥ 75% coverage | | |
