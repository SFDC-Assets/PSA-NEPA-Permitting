...
**Expected:**
- **All tests pass**, 0 failures. The current baseline is 666+ test methods across 65 test classes; the exact count increases as tests are added. When using `--result-format json`, a passing run shows `"summary": { "outcome": "Passed", "failing": 0 }` and the `summary.passing` field shows the current count.
