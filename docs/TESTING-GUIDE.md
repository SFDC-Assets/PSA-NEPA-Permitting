...
**Test suite size:** 27 test classes, 385+ test methods across all feature areas.
...
**Tests:** REST endpoint returns valid 9-entity payload; all CEQ standard fields present; schema version correct; PIC v1.2 compliance verified via Apex test class.
...
### 17d. PIC / MFR Compliance — Apex Test Suite

`NepaCeqExportServiceTest` contains 9 compliance tests that verify the export payload meets CEQ Permitting Data and Technology Standard v1.2 constraints. Run them with:
...
**Expected:** All 22 tests pass (13 functional + 9 compliance). The 9 compliance tests verify:
...
**Tests:** Completeness score computed; gaps identified; completeness updates on document addition. Covered by `NepaAdminRecordCheckerTest` (7 tests).
...
### 18b. Apex Test Suite — AR Checker

```bash
sf apex run test \
  --class-names NepaAdminRecordCheckerTest \
  --target-org $ALIAS \
  --result-format human \
  --wait 10
```

**Expected:** All 7 tests pass. The tests cover: approved document triggers scoring, score higher than no-docs baseline, `nepa_missing_documents__c` populated when docs absent, EIS `Draft EIS` CMT entry present, CE Determination CMT entry present, draft document does not count as approved, and bulk-approved docs without governor limit errors.

> **Note:** `NepaAdminRecordCheckerTest` and `NepaDefensibilityGapCheckerTest` tests that insert `ContentVersion` records may encounter `Internal Salesforce Error: 723447963` in the NEPADEMO sandbox — a pre-existing platform bug with `ContentVersion` inserts in that specific org. All tests include `try/catch` guards so this error does not mask other failures. The tests pass cleanly in orgs without this bug.
...
| `NepaBREConfigTest` | 39 | BRE Expression Sets, CMT integrity |
| `NepaEntity789Test` | 25 | GIS data, team members, legal structure |
| `NepaCeqExportServiceTest` | 22 | REST export API + PIC/MFR compliance |
...
| `NepaLitigationRiskScorerTest` | 16 | BRE risk scoring, tier thresholds |
| `NepaCommentTriageParserTest` | 16 | Comment triage parser |
| `NepaCommentEJDetectorTest` | 16 | EJ keyword detection |
| `NepaValidationRuleTest` | 15 | Field validation rules |
...
| `NepaPlaintiffIntelligenceTest` | 12 | Plaintiff risk flag, tribal dual-flag |
| `NepaDocumentControllerTest` | 9 | Document upload and query |
| `NepaAIGovernanceTest` | 9 | AI governance and AUP guardrails |
| `NepaDefensibilityGapCheckerTest` | 8 | Defensibility gap detection |
| `NepaTimelineControllerTest` | 7 | Timeline LWC controller |
| `NepaAdminRecordCheckerTest` | 7 | Administrative record completeness |
| `NepaEngagementControllerTest` | 6 | Engagement event controller |
| `NepaCeScreenerTest` | 6 | CE screening and intake |
| `NepaAIGovernanceFlowTest` | 6 | AI governance flow integration |
| `NepaChallengePredictorTest` | 5 | Challenge prediction rules |
| `NepaAgencyTierSetterTest` | 5 | Agency performance tier |
| `NepaRfpRequirementsTest` | 3 | RFP requirements coverage |
| `NepaGISProximityIPInvokerTest` | 3 | GIS proximity IP invoker |
| `NepaErrorHandlingTest` | 3 | Error handling architecture |
...
**Expected:** All 39 BRE config tests pass, including verification that all CMT types have at least one active record, statute weight point values match PermitTEC corpus calibration, and required document registry entries exist for CE, EA, and EIS pathways.
...
**Expected:** All 22 tests pass (13 functional + 9 PIC compliance). See Section 17d for the full compliance test inventory.
...
**Expected:** All 12 tests pass, covering VERY_HIGH org match, HIGH match, CONTAINS partial match, no-match isolation, blank org skip, tribal dual-flag, task subject content, and bulk insert.

### 20h. Targeted Test — Stage Gate
...
| 17d | CEQ Export — PIC Compliance | `compliance_federalUniqueId_nonNullOnExportedRecord` | | |
| 17d | CEQ Export — PIC Compliance | `compliance_reviewType_isValidPicValue` | | |
| 17d | CEQ Export — PIC Compliance | `compliance_processStatus_isValidPicValue` | | |
| 17d | CEQ Export — PIC Compliance | `compliance_envelopeShape_listResponseAlwaysArray` | | |
| 17d | CEQ Export — PIC Compliance | `compliance_envelopeShape_singleResponseIsObject` | | |
| 17d | CEQ Export — PIC Compliance | `compliance_errorEnvelope_hasRequiredFields` | | |
| 17d | CEQ Export — PIC Compliance | `compliance_allDtoFields_matchPicPropertyNames` (15 keys) | | |
| 17d | CEQ Export — PIC Compliance | `compliance_agencyId_nonNullWhenSet` | | |
| 18a | AR Completeness | Completeness score computed on demand | | |
| 18b | AR Completeness | NepaAdminRecordCheckerTest (7 tests) passes | | |
...
| 20a | Test Suite | All 385+ tests pass, ≥ 75% coverage | | |
| 20b | Test Suite | NepaBREConfigTest (39 tests) passes | | |
| 20c | Test Suite | NepaApiComplianceTest (55 tests) passes | | |
| 20d | Test Suite | NepaEntity789Test (25 tests) passes | | |
| 20e | Test Suite | NepaCeqExportServiceTest (22 tests) passes | | |
| 20f | Test Suite | NepaSlaEscalationMonitorTest (12 tests) passes | | |
| 20g | Test Suite | NepaPlaintiffIntelligenceTest (12 tests) passes | | |
| 20h | Test Suite | NepaStageGateTest (17 tests) passes | | |
