...
| Picklist conformance | 2 | `nepa_review_type` ∈ `{EIS, EA, CE, Other Authorization}`; `status` ∈ `{planned, pre-application, in progress, paused, completed, cancelled}` |
...
| 18a | CEQ Compliance | NepaCeqComplianceTest (38 tests) passes — picklists, provenance, DTO shapes, nesting | | |
