...
### 4.8 Implementation Decisions Derived from Stages 14–16

The three extended stages (14–16) produced findings that directly shaped platform implementation decisions in the PSA-NEPA accelerator. This section documents those research-to-implementation mappings.

**Stage 14 → v3 Cost Exposure Dimension (IMP-001).** Section 4.6 establishes that litigation duration is a cost dimension, not a probability signal (median 15.1 months for agency wins vs. 16.3 months for challenger wins). Prior to v3, the composite risk score was a single probability estimate; project sponsors making financing and sequencing decisions used it as a proxy for expected cost. This is incorrect. A low-probability / high-duration agency (FTA: 33.4 months, n=4) imposes greater expected cost than a high-probability / short-duration agency (BOEM: 6.5 months, 100% win rate) when the underlying probability difference is small. The v3 score bifurcates into a Litigation Probability Score (85% weight) and a Litigation Cost Exposure dimension (15% weight), stored in `nepa_litigation_duration_cost__c`. The normalized cost dimension scales the per-agency median duration against the observed range [BOEM=6.5 mo, FTA=33.4 mo]. The `nepaRiskIntelligenceCard` LWC displays both dimensions separately with tooltips explaining that cost and probability are independent risk axes.

**Stage 15 → ESA Low-Confidence Disclosure (IMP-004).** Section 4.4 documents that the ESA §7 multiplier (1.48×) cannot currently be decomposed into consultation-duration or jeopardy-finding components because ECOS provides no programmatic API and Stage 15 returned zero ECOS matches. The multiplier is retained at 1.48× as the best available estimate from observed PermitTEC outcomes, but it is not derivable from the causal mechanism (consultation duration → outcome probability) that drives the other statute weights. The implementation response is to set `Low_Data_Confidence__c = true` on the ESA `NEPA_Statute_Risk_Weight__mdt` record and configure the `nepaRiskIntelligenceCard` LWC to display a yellow disclosure banner when ESA is a contributing factor in the score: *"ESA statute weight uses flat 1.48× — pending TAILS/PCTS linkage."* This disclosure is consistent with OMB M-24-10's requirement that automated scoring outputs disclose known data limitations at point-of-use rather than in documentation that users will not read at the moment of decision.

**Stage 16 → OFD Coordination Tracker and Federal Friction Surfacing (IMP-005, IMP-006).** Section 4.3 establishes sector-specific federal friction multipliers: Military 1.65×, Water/Coastal 1.47×, Transportation 1.45×, Energy 1.09×. The dominant drivers of the Military and Water/Coastal premiums are multi-agency coordination overhead — NHPA Section 106 multi-installation review (Military) and CZMA consistency + EFH Magnuson-Stevens dual-track review (Water/Coastal). These findings have two implementation consequences:

1. **Sector-specific extraordinary circumstances intake fields (IMP-008).** Fields `nepa_ec_multi_dod__c` (Military multi-installation DoD coordination required) and `nepa_ec_usace_czma__c` (Water/Coastal CZMA consistency + EFH dual-track review required) are present on `IndividualApplication` and exposed via the `NEPA_CE_Intake` Screen Flow (verified). Note: CE intake OmniScript (`NEPA_CEIntake_OmniScript_1`) is a design artifact in the repository — these fields are present in the metadata but the OmniScript path has not been verified. See [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail). These fields capture the primary multi-agency coordination triggers at intake, enabling coordinators to see the friction multiplier basis before the first field survey.
