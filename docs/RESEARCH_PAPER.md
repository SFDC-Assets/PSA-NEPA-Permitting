...
1. **Sector-specific extraordinary circumstances intake fields (IMP-008).** Fields `nepa_ec_multi_dod__c` (Military multi-installation DoD coordination required) and `nepa_ec_usace_czma__c` (Water/Coastal CZMA consistency + EFH dual-track review required) are present on `IndividualApplication` and exposed via the `NEPA_CE_Intake` Screen Flow (verified). Note: CE intake OmniScript (`NEPA_CEIntake_OmniScript_1`) is a design artifact in the repository — these fields are present in the metadata but the OmniScript path has not been verified. See [OMNISTUDIO-BACKLOG.md](OMNISTUDIO-BACKLOG.md). These fields capture the primary multi-agency coordination triggers at intake, enabling coordinators to see the friction multiplier basis before the first field survey.
