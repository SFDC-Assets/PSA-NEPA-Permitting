...
> **Note:** An OmniStudio Integration Procedure (`NEPA_CEQExport_Procedure`) and DataRaptor Extract chain are present in the repository as design artifacts but were not successfully verified (abandoned — unresolvable "valid bundle name" error in the OmniStudio platform). The working export paths are the two Apex REST endpoints. See [OMNISTUDIO-BACKLOG.md](OMNISTUDIO-BACKLOG.md).
...
> **Note:** A 7-step OmniScript CE Intake Wizard (`NEPA_CEIntake`) backed by Integration Procedures is present in the repository but was not successfully deployed and verified. The working intake path is the `NEPA_CE_Intake` Screen Flow. See [OMNISTUDIO-BACKLOG.md](OMNISTUDIO-BACKLOG.md).
...
**Integration:** The `NEPA/CEQExport` REST API (Apex-based) exposes all 13 entities in PIC OpenAPI v1.2.0 format, including a `permits[]` node. GIS proximity checks against FWS ECOS, EPA EJScreen, USGS NHD, BLM tribal cadastral, and BLM PLSS are implemented via an Apex bridge (`NepaGISProximityIPInvoker`) and `NEPA_GIS_Proximity_Check` Flow. Note: the `NEPA_GISProximityIP` OmniStudio Integration Procedure that backs the Apex bridge is present in the repository but has not been successfully verified end-to-end. See [OMNISTUDIO-BACKLOG.md](OMNISTUDIO-BACKLOG.md). The `nepaPermitDependencies` LWC calls other agencies' deployed NEPA REST endpoints at record load, providing live cross-agency permit status without leaving Salesforce. Both the GIS and agency permit integration patterns extend by adding Named Credentials and metadata records — no Apex required.
...
> **Note:** A 7-step OmniScript CE Intake Wizard designed for this flow is in the repository but has not been verified. The working applicant intake path is the Screen Flow. See [OMNISTUDIO-BACKLOG.md](OMNISTUDIO-BACKLOG.md).
...
**Section 508 / WCAG 2.1 AA compliance** is inherited from Salesforce Lightning Design System components. The delivered Screen Flow and LWC components follow SLDS accessibility standards. **Note:** OmniScript components are backlog — accessibility compliance for those components has not been verified; see [OMNISTUDIO-BACKLOG.md](OMNISTUDIO-BACKLOG.md).
