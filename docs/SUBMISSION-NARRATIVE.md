...
**MFR #1 — Data Standards (Leading-Edge).** All 13 CEQ entities are implemented on Salesforce-native objects with required standard fields, provenance fields, and the `nepa_other__c` extension bag. The `NEPA/CEQExport` REST API exposes all 13 entities in PIC OpenAPI v1.2.0-aligned JSON; authorized systems can read all 13 entities via the REST endpoint — meeting Leading-Edge maturity. A 138-test regression suite (`NepaApiComplianceTest`, `NepaCeqExportServiceTest`, `NepaCeqFullExportServiceTest`) verifies compliance continuously. **Note:** An Upsert DataRaptor integration is present in the repository as a design artifact (backlog — not verified); see [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail).

**MFR #2 — Application Data Sharing (Emerging).** Two complementary Apex REST endpoints deliver the CEQ v1.2 export payload: `NepaCeqExportService` (`GET /services/apexrest/nepa/v1/processes/{id}`) for per-process interoperability (used by cross-agency permit callouts), and `NepaCeqFullExportService` (`POST /services/apexrest/nepa/v1/export/project`) for the complete project graph — Project → Processes → Documents + Comments, Engagement Events, Case Events, Team Members, GIS Data, and Permits, in CEQ v1.2 snake_case property names. Together these expose all 13 entities as a structured payload, providing the structured dependent permit data to consuming systems without data extraction overhead. The same endpoints serve EPA DARTER, USACE ORM2, DOT NEPA tracking systems, and FPISC. Information entered once is available to all authorized downstream systems without re-entry.

> **Note:** An OmniStudio Integration Procedure (`NEPA_CEQExport_Procedure`) and DataRaptor Extract chain are present in the repository as design artifacts but were not successfully verified (abandoned — unresolvable "valid bundle name" error in the OmniStudio platform). The working export paths are the two Apex REST endpoints. See [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail).

**MFR #3 — Automated Project Screening (Leading-Edge).** The three-tier BRE CE Screener evaluates the project against 2,105 CE authorities across 79 agencies, applies all six extraordinary circumstances triggers, and returns a recommendation with the specific rule row that fired — before formal submission. GIS proximity checks fire automatically against all active layers in the `NEPA_GIS_Layer__mdt` registry (15 services). Leading-Edge maturity: screening criteria and GIS data are publicly available via the GitHub decision model exports, and the screening logic is integrated with the intake process.

> **Note:** A 7-step OmniScript CE Intake Wizard (`NEPA_CEIntake`) backed by Integration Procedures is present in the repository but was not successfully deployed and verified. The working intake path is the `NEPA_CE_Intake` Screen Flow. See [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail).
...
**Integration:** The `NEPA/CEQExport` REST API (Apex-based) exposes all 13 entities in PIC OpenAPI v1.2.0 format, including a `permits[]` node. GIS proximity checks against FWS ECOS, EPA EJScreen, USGS NHD, BLM tribal cadastral, and BLM PLSS are implemented via an Apex bridge (`NepaGISProximityIPInvoker`) and `NEPA_GIS_Proximity_Check` Flow. Note: the `NEPA_GISProximityIP` OmniStudio Integration Procedure that backs the Apex bridge is present in the repository but has not been successfully verified end-to-end. See [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail). The `nepaPermitDependencies` LWC calls other agencies' deployed NEPA REST endpoints at record load, providing live cross-agency permit status without leaving Salesforce. Both the GIS and agency permit integration patterns extend by adding Named Credentials and metadata records — no Apex required.
...
**Applicants** are guided through CE intake via the `NEPA_CE_Intake` Screen Flow, which captures federal jurisdiction, project sector and type, action type (the primary CE/EA discriminator), physical parameters, NAICS code, and GIS footprint. Applicants without GIS shapefiles can enter decimal-degree coordinates; the system geocodes to a centroid and runs proximity checks against that point. GIS proximity checks fire automatically at submission against all 15 registered layers. If a GIS endpoint is unavailable, that layer's result is marked for manual review and a High-priority coordinator Task is created; the remaining checks complete and the application proceeds. Applicants receive a CE pre-screening result — recommended review type, applicable CE code set, confidence level, and extraordinary circumstances flags — before formal submission.

> **Note:** A 7-step OmniScript CE Intake Wizard designed for this flow is in the repository but has not been verified. The working applicant intake path is the Screen Flow. See [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail).
...
**Section 508 / WCAG 2.1 AA compliance** is inherited from Salesforce Lightning Design System components. The delivered Screen Flow and LWC components follow SLDS accessibility standards. **Note:** OmniScript components are backlog — accessibility compliance for those components has not been verified; see [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail).
...
**Current state: production-ready.** The accelerator is fully deployed and verified against the CEQ PIC Standard v1.2.0. A 628+ test Apex regression suite covers all 13 entities, both REST export APIs (per-process and full project graph, including permits[] node), BRE configuration integrity, CE screening, stage gate logic, SLA escalation, plaintiff intelligence, EJ detection, GIS proximity, comment agent routing, cross-agency permit callouts, permit SLA monitoring, FLS enforcement for the `nepa_required_permit__c` object, and error handling. All tests pass. Code coverage exceeds 75%.
...
```
No infrastructure provisioning, no database migration, no vendor onboarding. The repository includes complete object definitions, 40+ flow XML files, permission sets with field-level security, DMN decision model exports for all 8 Decision Matrices, custom metadata pre-seeded with empirically calibrated risk weights, and two Lightning Web Components: `nepaPermitDependencies` (live cross-agency permit status) and `nepaRiskIntelligenceCard` (bifurcated litigation risk display). The repository also includes 15 DataRaptor files and 6 Integration Procedure files (OmniStudio components) that are present as design artifacts but have not been successfully verified — these are backlog items. See [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail).
...
**CEQ standard REST API.** Two Apex REST services expose the full CEQ v1.2 payload. `NepaCeqExportService` (`GET /services/apexrest/nepa/v1/processes/{id}`) provides per-process export for cross-agency interoperability — each agency's NEPA platform calls this endpoint to retrieve live permit status from a partner agency's NEPA deployment. `NepaCeqFullExportService` (`POST /services/apexrest/nepa/v1/export/project`) produces the complete project graph: Project → all Processes → Documents + Comments, Engagement Events, Case Events, Team Members, GIS Data, and Permits — in a single bulk-safe call (11 SOQL queries, all outside loops). All output keys use CEQ v1.2 snake_case property names. EPA DARTER, USACE ORM2, DOT NEPA tracking systems, and any internal permit database can pull structured NEPA data via authenticated REST call without a custom extraction step. (Note: a `NEPA_CEQExport` OmniStudio Integration Procedure is present in the repository as a design artifact but was abandoned — see [ARCHITECTURE_DECISIONS.md — Appendix C](ARCHITECTURE_DECISIONS.md#appendix-c--omnistudio-backlog-detail).)
