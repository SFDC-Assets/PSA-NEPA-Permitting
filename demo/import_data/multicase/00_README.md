...
Five additional NEPA cases extracted from the `nepadata/samples` corpus, covering all three review types across three agencies. These supplement the primary Carrie Placer Mine case study (in `demo/import_data/`) with a diverse portfolio.
...
Each `case_X_*/` directory contains:
...
| `10_ContentVersion.csv` | ContentVersion | 1–3 | `nepa_document_key__c` = `SAMPLE-DOC-<UUID8>` |
...
```bash
TARGET=NEPADEMO
CASE=demo/import_data/multicase/case_A_ce_blm
...
sf data upsert bulk -s ContentVersion        -f "$CASE/10_ContentVersion.csv"        -i nepa_document_key__c        -o $TARGET -w 10
